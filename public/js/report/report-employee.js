...
function initStatistikKehadiran() {
  $.ajax({
    ...
    success: function (res) {
      ...
      $("#total-leave").children("span:nth-child(2)").text(`${stat.cuti}x`);
      $("#total-permit").children("span:nth-child(2)").text(`${stat.izin}x`);
      $("#verification")
        .children("span:nth-child(2)")
...
function initHolidaysThenTable() {
  $.ajax({
    url:
      base_url() + `report/ajax/get-holidays/${periode.start}/${periode.end}`,
    type: "GET",
    dataType: "json",
...
function initTable() {
  tableReport = $(".table-report").DataTable({
    ...
    // serverSide: true,
    fixedHeader: true,
    ajax: base_url() + `report/ajax/datatable/report-employee/${employee_id}`,
    dom: `t`,
    columns: [
      // 0. Date
      {
      ...
      // 3. Scan Masuk
      {
        ...
        render: function (data, type, row) {
          ...
          if (row.verifikasi == "scan_masuk" || row.verifikasi == "hadir") {
      ...
      // 5. Scan Keluar
      {
        ...
        render: function (data, type, row) {
          ...
          if (row.verifikasi == "scan_keluar" || row.verifikasi == "hadir") {
            return `<div class="btn btn-sm btn-success" onclick="showVerifikasi('${row.verifikasi_id}')"> ${data}</div>`;
          }
          return data;
        },
    ...
    rowCallback: function (row, data) {
      if (data.is_cuti == 1) {
        ...
        } else if (data.scan_masuk == null || data.scan_keluar == null) {
  ...
  if (report_id != 0 && check_holiday == false) {
    $.ajax({
      ...
      success: function (res) {
        ...
        } else if (report.is_lembur == 1 || report.is_lembur_libur == 1) {
          console.log(3);
          $("#lembur-tab").hide();
          $("#izin-tab").show();
          $("#izin-tab").tab("show");
...
function validateFile(input) {
  ...
  if (file) {
    ...
    if (!allowedTypes.includes(file.type)) {
      ...
      input.value = "";
      $(input).next(".custom-file-label").text("Choose file (pdf/jpeg/png)");
      return false;
    }
