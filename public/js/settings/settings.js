    ...
    case "holidays":
      if (!tableHolidays) {
        tableHolidays = $(".table-holidays").DataTable({
          processing: true,
          serverSide: true,
          ajax: base_url() + `settings/ajax/datatable/holidays/${yearHoliday}`,
          // dom: `tip`,
          dom: `t<"row mt-3"<"col-sm-12 col-md-5"i><"col-sm-12 col-md-7"p>>`,
          columns: [
            {
...
function editShift(id) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      $("#edit-jam_selesai_istirahat-shift").val(res.jam_selesai_istirahat);
      ...
      // Extra break: toggle + rows
      const isBreak = res.is_break == 1;
      const isExtra = res.is_break_extra == 1;
      $("#edit-is_break_extra").prop("checked", isExtra);

      if (isBreak) {
        $("#edit-extraBreakToggleWrapper").show();
      } else {
        $("#edit-extraBreakToggleWrapper").hide();
      }

      const $container = $("#edit-extraBreakContainer");
      const $fields = $("#edit-extraBreakFields");
      const $addBtn = $("#edit-addExtraBreakBtn");
      $container.empty();

      if (
        isBreak &&
        isExtra &&
        Array.isArray(res.istirahat) &&
        res.istirahat.length > 0
      ) {
        res.istirahat.forEach((item) => {
          const mulai = item && item.mulai ? item.mulai : "";
          const selesai = item && item.selesai ? item.selesai : "";
          const $row = $(
        ...
        // remove handler (rebind cleanly)
        $container
          .off("click", ".edit-remove-extra-break")
          .on("click", ".edit-remove-extra-break", function () {
            $(this).closest(".extra-break-row").remove();
          });
        if ($.fn && $.fn.timepicker) {
      ...
      if (isBreak && isExtra) {
        $fields.show();
        if ($addBtn.length) $addBtn.prop("disabled", false);
      } else {
        $fields.hide();
        if ($addBtn.length) $addBtn.prop("disabled", true);
      }

      // Trigger change so any local listeners in the modal resync UI
      $("#edit-is_break-shift").trigger("change");
      $("#edit-is_break_extra").trigger("change");
    },
  });
}
...
function handleIsBreak() {
  if (document.getElementById("edit-is_break-shift").checked) {
    document.getElementById("edit-jam_mulai_istirahat-shift").disabled = false;
    document.getElementById(
      "edit-jam_selesai_istirahat-shift"
    ).disabled = false;
  } else {
    document.getElementById("edit-jam_mulai_istirahat-shift").disabled = true;
    document.getElementById("edit-jam_selesai_istirahat-shift").disabled = true;
  }
}
...
function holidaysArea() {
  ...
  $(".yearpicker").yearpicker({
    onChange: function (value) {
      yearHoliday = value;
      reloadTableHolidays();
      $(".yearpicker").html(`<i class="far fa-calendar mr-1"></i> ${value}`);
    },
    endYear: yearHoliday,
  });
  $(".yearpicker").html(`<i class="far fa-calendar mr-1"></i> ${yearHoliday}`);
...
function showDetailShift(id) {
  $.ajax({
    ...
    success: function (res) {
      ...
      $("#show-jam_selesai_istirahat-shift").val(res.jam_selesai_istirahat);
